...
blsa_template = os.path.join(template_dir,'BLSA_avg_temp_space',
                'BLSA_MPRAGE_AllBaselines_Age60-80_Random100_averagetemplate_final_short_reoriented.nii.gz')
...
# Identify MUSE 3T MPRAGEs (scanners 8, 9, 10)
mprage3T = np.zeros(data_table.shape[0], dtype=bool)
for i, row in data_table.iterrows():
    if int(os.path.basename(row['musemripath'])[15:17]) in [8,9,10]:
        mprage3T[i] = True

data_table = data_table[mprage3T]
...
# Identify baseline MUSE 3T MPRAGEs
...
# Identify non-baseline MUSE 3T MPRAGEs
...
data_table_nonbaseline.to_csv(os.path.join(output_dir,'baseline_mapping_mprage3T.csv'),index=False)
...
# Save outputs of interest: reoriented baseline 3T MPRAGE, transform to template, baseline MPRAGE in template space,
#                           baseline MPRAGE in MNI space
...
# Step 3: Deformably register follow-up MPRAGE to baseline MPRAGE
...
# Compute within-subject registration onto baseline MPRAGE
...
# Step 4: Concatenate transforms and apply to follow-up MPRAGE to bring it into template space and into MNI space
reverselist = Node(Function(input_names=['l'],output_names=['revlist'],function=reverse_list),
                   name='reverselist')
